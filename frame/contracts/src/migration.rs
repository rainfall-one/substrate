...
use crate::{Config, Error, MigrationInProgress, Pallet, Weight};
...
trait MigrateSequence<T: Config> {
	...
	fn new_weight(version: StorageVersion) -> Weight;
...
impl<T: Config> OnRuntimeUpgrade for Migration<T> {
	fn on_runtime_upgrade() -> Weight {
		...
		weight.saturating_accrue(Migrations::<T>::new_weight(storage_version + 1));
...
impl<T: Config> Migration<T> {
	...
	pub(crate) fn migrate(weight_limit: Weight) -> Result<Weight, (Weight, DispatchError)> {
		...
		MigrationInProgress::<T>::try_mutate_exists(|progress| {
			...
			let next_version = in_progress_version + 1;
			let next_version_exists = <Pallet<T>>::current_storage_version() != in_progress_version;

			// if we finish this version we might need to create the next version
			// we don't know if that will be the case until after we ran the steps
			if next_version_exists {
				weight_left
					.checked_reduce(Migrations::<T>::new_weight(next_version))
					.ok_or_else(|| {
						(
							weight_limit.saturating_sub(weight_left),
							Error::<T>::NoMigrationPerformed.into(),
						)
					})?;
			}
			...
			*progress = match Migrations::<T>::steps(
				...
				None => {
					...
					if next_version_exists {
						...
						Some(Migrations::<T>::new(next_version))
					} else {
						// refund as we did not call the `new` function
						weight_left.saturating_accrue(Migrations::<T>::new_weight(next_version));
						// all migrations done: enable pallet by removing the storage item
						None
					}
				},
			};
			...
			#(
				if version == Tuple::VERSION {
					let mut migration = <Tuple as Decode>::decode(&mut cursor)
						.expect(PROOF_DECODE);
					let max_weight = Tuple::step_weight();
