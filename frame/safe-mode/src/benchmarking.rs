...
use frame_support::{assert_ok, traits::{fungible::Mutate as FunMutate, UnfilteredDispatchable}};
...
mod benchmarks {
	...
	fn release_stake() -> Result<(), BenchmarkError> {
		...
		System::<T>::set_block_number(
			delay + One::one() + 2u32.into(),
		);
		System::<T>::on_initialize(System::<T>::block_number());
		SafeMode::<T>::on_initialize(System::<T>::block_number());

		let call =
			Call::<T>::release_stake { account: alice.clone(), block: 1u32.into() };
		...
		assert_eq!(!Stakes::<T>::iter_keys().count(), 0);
